...
const SimpleDashboard = () => {
  ...
  if (loading) {
    return (
      <Box xcss={{ padding: 'space.500', textAlign: 'center' }}>
        <Text xcss={{ fontSize: '1.2em', color: '#0052CC', marginBottom: 'space.200' }}>🧠 Loading Organizational Intelligence...</Text>
        <Text xcss={{ color: '#6B7280' }}>Analyzing Jira execution patterns</Text>
      </Box>
    );
  }
  ...
  if (error) {
    return (
      <Box xcss={{ padding: 'space.500', textAlign: 'center' }}>
        <Text xcss={{ fontSize: '1.2em', color: '#DE350B', marginBottom: 'space.200' }}>⚠️ Productivity Intelligence Unavailable</Text>
        <Text xcss={{ color: '#6B7280', marginBottom: 'space.200' }}>{error}</Text>
        <Text xcss={{ color: '#6B7280', fontSize: '0.9em' }}>Create some Jira tasks through MeetingMind to see insights</Text>
        <Button onClick={loadDashboardData} appearance="primary" xcss={{ marginTop: 'space.200' }}>
          Retry Analysis
        </Button>
      </Box>
    );
  ...
  return (
    <Stack space="large">
      ...
      <Box xcss={{
        ...
        <Box xcss={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'space.200', marginBottom: 'space.200' }}>
          <Text xcss={{ fontSize: '2em' }}>📊</Text>
          <Heading size="xlarge" xcss={{ color: 'white' }}>Jira Pulse Dashboard</Heading>
        </Box>
        <Text xcss={{ color: 'white', opacity: 0.9, fontSize: '1.1em' }}>
          Organizational productivity intelligence — not who works hardest, but who delivers value.
        </Text>
      </Box>
      ...
      {activeTab === 'individual' && (
        <Stack space="large">
          ...
          <Box xcss={{
            ...
            <Heading size="medium" xcss={{ marginBottom: 'space.400' }}>💡 Personal Improvement Suggestions</Heading>
            <Stack space="medium">
              <Box xcss={{
                padding: 'space.400',
                borderRadius: 'border.radius.200',
                ...
                <Text xcss={{ fontWeight: '600', color: '#0052CC', marginBottom: 'space.100' }}>Consider batching similar tasks</Text>
                <Text xcss={{ fontSize: '0.9em', color: '#6B778C' }}>
                  Your context switching frequency is 20% higher than optimal. Try time-blocking for deep work.
                </Text>
              </Box>
              <Box xcss={{
                padding: 'space.400',
                ...
                <Text xcss={{ fontWeight: '600', color: '#00875A', marginBottom: 'space.100' }}>Great job on meeting commitments</Text>
                <Text xcss={{ fontSize: '0.9em', color: '#6B778C' }}>
                  Your on-time delivery rate has improved 6% this month. Keep it up!
                </Text>
              </Box>
            </Stack>
          </Box>
...
const GlassMetricCard = ({ title, value, change, icon, color, subtitle }) => {
  ...
  return (
    <Box xcss={{
      ...
      <Box xcss={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'space.300' }}>
        <Box xcss={{
          width: '48px',
          height: '48px',
          borderRadius: 'border.radius.200',
      ...
      <Box>
        <Text xcss={{ fontSize: '0.9em', color: '#6B778C', marginBottom: 'space.100' }}>{title}</Text>
        <Text xcss={{ fontSize: '2.2em', fontWeight: '800', color: '#172B4D', lineHeight: '1' }}>{value}</Text>
        {subtitle && (
          <Text xcss={{ fontSize: '0.8em', color: '#6B778C', marginTop: 'space.100' }}>{subtitle}</Text>
        )}
      </Box>
    </Box>
  );
  ...
  return (
    <Box xcss={{
      ...
      <Box xcss={{ marginBottom: 'space.500' }}>
        <Heading size="medium" xcss={{ marginBottom: 'space.100' }}>👥 Team Productivity Profiles</Heading>
        <Text xcss={{ color: '#6B778C', fontSize: '0.9em' }}>Individual insights without rankings • Contextual, not comparative</Text>
      </Box>
          ...
          <Box key={index} xcss={{
            ...
            <Box xcss={{
              ...
              <Text xcss={{ color: 'white' }}>
                {profile.initials || profile.name?.substring(0, 2).toUpperCase() || '??'}
              </Text>
            </Box>
            
            <Box xcss={{ flex: 1 }}>
              <Box xcss={{ display: 'flex', alignItems: 'center', gap: 'space.200', marginBottom: 'space.100' }}>
                <Text xcss={{ fontWeight: '700', fontSize: '1.1em', color: '#172B4D' }}>
            ...
            <Box xcss={{ 
              ...
              <Box xcss={{ textAlign: 'center' }}>
                <Text xcss={{ fontWeight: '700', color: '#0052CC', fontSize: '1.3em', display: 'block' }}>
                  {profile.reliability || 0}%
                </Text>
                <Text xcss={{ color: '#6B778C', fontSize: '0.8em' }}>Reliability</Text>
              </Box>
              <Box xcss={{ textAlign: 'center' }}>
                <Text xcss={{ fontWeight: '700', color: '#00875A', fontSize: '1.3em', display: 'block' }}>
                  {profile.quality || 0}%
                </Text>
                <Text xcss={{ color: '#6B778C', fontSize: '0.8em' }}>Quality</Text>
              </Box>
              <Box xcss={{ textAlign: 'center' }}>
                <Text xcss={{ fontWeight: '700', color: '#6554C0', fontSize: '1.3em', display: 'block' }}>
                  {profile.collaboration || 0}%
                </Text>
                <Text xcss={{ color: '#6B778C', fontSize: '0.8em' }}>Impact</Text>
              </Box>
            </Box>
          </Box>
        ))}
...
const InsightCard = ({ type, title, description, metric }) => {
  ...
  return (
    <Box xcss={{
      ...
      <Box xcss={{ flex: 1 }}>
        <Text xcss={{ fontWeight: '700', marginBottom: 'space.100', color: '#172B4D' }}>{title}</Text>
        <Text xcss={{ color: '#6B778C', lineHeight: '1.4' }}>{description}</Text>
        {metric && (
          <Text xcss={{ 
            color: getColor(), 
            fontWeight: '600', 
...
const DimensionBar = ({ label, value, color }) => (
  <Box xcss={{ marginBottom: 'space.300' }}>
    <Inline space="space.200" alignBlock="center">
      <Text xcss={{ fontWeight: '600', minWidth: '180px', color: JIRA_COLORS.text }}>{label}</Text>
      <Box xcss={{
        flex: 1,
        height: '16px',
      ...
      <Text xcss={{ fontWeight: '700', color: color, minWidth: '60px', textAlign: 'right' }}>
        {value}%
      </Text>
    </Inline>
  </Box>
    ...
    <Stack space="medium">
      ...
      >
        ...
        <Text xcss={{ color: 'white', opacity: 0.9 }}>
          Transform meetings into tasks • Analyze productivity • Optimize organizational performance
        </Text>
